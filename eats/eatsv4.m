function [] = eatsv4( Sim_Time, Nodes, Channels, Queue_Size, Lambda, Packet_Size)
...
while flag
    ...
    elseif event==2        
        [T,Event_List, Sending_list, CAT, RAT, D, Senders_list, index, Sending_times]=Event2(T,Event_List, Sending_list, Q1, Q2, CAT, RAT, Channels, sync_time, Rtt, D, Nodes, Senders_list, index, Queue_Size, Sending_times);
    
    elseif event == 3 %check for  packets
        [T, Event_List, Sending_times] = Event3(T, Event_List, Sending_list, Senders_list, Channels, Nodes, Q1, Q2, Sending_times);
        
    elseif event == 4
        [T, Event_List, Sending_list, Senders_list, Q1, Q2, index, CAT, RAT] = Event4(T, Event_List, Sending_list, Senders_list, Q1, Q2, Channels, Queue_Size, index, CAT, RAT);
...
function [T,Event_List, Sending_list, CAT, RAT, D, Senders_list, index, Sending_times]=Event2(T,Event_List, Sending_list, Q1, Q2, CAT, RAT, Channels, sync_time, Rtt, D, Nodes, Senders_list, index, Queue_Size, Sending_times)
fprintf('\nevent 2 time %d\n', Event_List(2,1));
%disp('queues')
Q1
Q2
...
%    fprintf('sender %d\nchannel %d\n', Event_List(3,1), chosen_channel);
    ...
    for i = 1 : Nodes %for each destination
        if D(Event_List(3,1), i) ~= 0
            ...
            Sending_times(Event_List(3, 1), (t2 - Rtt)) = T + (t2 - Rtt); %add the time 
...
function [T, Event_List, Sending_times] = Event3(T, Event_List, Sending_list, Senders_list, Channels, Nodes, Q1, Q2, Sending_times)
T = Event_List(2, 1);
fprintf('\nevent 3 time %f\n', T);
Sending_list
Sending_times
L = size(Sending_times);
Sending_times((1 : Nodes), 2 : end) = Sending_times((1 : Nodes), 1) + (1 : L(2) -1) %mark the times
%disp(Sending_list)
for times = 1 : L(2) %run the times, hor
    for stations = 1 : Nodes
        if Sending_times(stations, times) ~= 0
            Event_List(1, end + 1) = 4; %send event
            Event_List(2, end) = Sending_times(stations, times);
        end
    end
end

Event_List(1, end + 1) = 3;
Event_List(2, end) = max(Sending_times) + Nodes * 0.25;

Event_List(1, end + (1:Nodes)) = 2;
Event_List(2, end - (0:Nodes - 1)) = max(Sending_times) + 0.25 * (1:Nodes);
...
function [T, Event_List, Sending_list, Senders_list, Q1, Q2, index, CAT, RAT] = Event4(T, Event_List, Sending_list, Senders_list, Q1, Q2, Channels, Queue_Size, index, CAT, RAT)
T = Event_List(2, 1);
fprintf('\nevent 4 time %f\n', T);
sender = 0;
disp('initial values')
