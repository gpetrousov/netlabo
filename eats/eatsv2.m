function [] = eatsv2( Sim_Time, Nodes, Channels, Queue_Size, Lambda, Packet_Size)
...
while flag
    ...
    elseif event==2        
        [T,Event_List, Sending_list, CAT, RAT, D, Senders_list]=Event2(T,Event_List, Sending_list, Q1, Q2, CAT, RAT, Channels, sync_time, Rtt, D, Nodes, Senders_list, Queue_Size);
    
    elseif event == 3 %check for  packets
        [T, Event_List] = Event3(T, Event_List, Sending_list, Senders_list, Channels, Nodes, Q1, Q2);
        
    elseif event == 4
        [T, Event_List, Sending_list, Senders_list, Q1, Q2, D, CAT, RAT] = Event4(T, Event_List, Sending_list, Senders_list, Q1, Q2, D, Channels, Queue_Size, CAT, RAT);
...
function [T,Event_List, Sending_list, CAT, RAT, D, Senders_list]=Event2(T,Event_List, Sending_list, Q1, Q2, CAT, RAT, Channels, sync_time, Rtt, D, Nodes, Senders_list, Queue_Size)
fprintf('\nevent 2 time %d\n', Event_List(2,1));
%disp('queues')
%Q1
%Q2
...
function [T, Event_List, Sending_list, Senders_list, Q1, Q2, D, CAT, RAT] = Event4(T, Event_List, Sending_list, Senders_list, Q1, Q2, D, Channels, Queue_Size, CAT, RAT)
T = Event_List(2, 1);
fprintf('\nevent 4 time %f\n', T);
sender = 0;
...
for ch = 1 : Channels
    if Sending_list(ch, 1) ~= 0 %someone is sending
        ...
        min_time = Q1(sender, 1);
        recv_pos = 1;        
        for j = 2 : Queue_Size
            if Q1(sender, j) ~= -1 && Q1(sender, j) < min_time
                min_time = Q1(sender, j);
                recv_pos = j;
            end            
        end
        %recv_pos = find(Q1(sender, :) == min(Q1(sender, :)))  %finds min time
                
        receiver = Sending_list(ch, 1)
        fprintf('channel %d\n', ch);
        Q2(sender, recv_pos) = 0;
        Q1(sender, recv_pos) = -1;
    end
end
Sending_list(:, 1) = [];
Senders_list(:, 1) = [];
